...
def get_data_loaders(batch_size, val_frac=0.1, seed=42):
    # 1) download train, compute mean/std on train split
    full_train = CIFAR10(root="data", train=True, download=True, transform=transforms.ToTensor())
    n_total = len(full_train)               # 50,000
    n_val   = int(val_frac * n_total)       # e.g. 5,000
    ...
    train_idx, val_idx = next(sss.split(np.zeros(n_total), targets))

    train_subset = Subset(full_train, train_idx)
    val_subset   = Subset(full_train, val_idx)

    # compute mean/std ON-THE-FLY or just use CIFAR_MEAN/CIFAR_STD constants
    mean, std = compute_mean_std(train_subset, batch_size)
    print(f"CIFAR‑10 mean: {mean}, std: {std}")

    # 2) define transforms
    normalize = transforms.Normalize(mean=mean, std=std)
    eval_transform = transforms.Compose([
        transforms.Resize(CIFAR_IMAGE_SIZE),
        transforms.CenterCrop(CIFAR_IMAGE_SIZE),
        transforms.ToTensor(),
        normalize,
    ])
    def get_augment():
        ...
        return transforms.Compose([
            transforms.RandomResizedCrop(CIFAR_IMAGE_SIZE, scale=(0.08,1.0)),
    ...
    # 3) wrap for SimCLR
    aug = get_augment()
    two_crop = TwoCropsTransform(aug)
    train_simclr = SimCLRDataset(train_subset, two_crop)

    # 4) plain eval datasets
    val_eval = LabeledEvalDataset(val_subset, eval_transform)
    test = CIFAR10(root="data", train=False, download=True, transform=transforms.Compose([
        transforms.Resize(CIFAR_IMAGE_SIZE),
        transforms.ToTensor(),
        normalize
    ]))
    test_eval = LabeledEvalDataset(test, eval_transform)

    # 5) loaders
    train_loader = DataLoader(train_simclr, batch_size=batch_size, shuffle=True, drop_last=True, num_workers=NUM_WORKERS)
    val_loader   = DataLoader(val_eval,    batch_size=batch_size, shuffle=False, num_workers=NUM_WORKERS)
    test_loader  = DataLoader(test_eval,   batch_size=batch_size, shuffle=False, num_workers=NUM_WORKERS)

    return train_loader, val_loader, test_loader, val_subset, NUM_CLASSES



def get_cifar_data_loaders(batch_size,
                           val_frac: float = 0.1,
                           seed: int = 42):
    """
    Returns:
      train_loader       -- yields (x1, x2) for SimCLR
      val_loader         -- yields (img, label) for validation
      test_loader        -- yields (img, label) for test
      val_subset_no_tf   -- Subset of raw PIL CIFAR for any further eval
      num_classes        -- always 10 for CIFAR‑10
    """
    # 1) Download raw PIL train+test
    raw_train = CIFAR10(root="data", train=True,  download=True, transform=None)
    raw_test  = CIFAR10(root="data", train=False, download=True, transform=None)

    # 2) Stratified split raw_train → train / val
    labels = np.array(raw_train.targets)
    n_total = len(raw_train)                 # 50_000
    n_val   = int(val_frac * n_total)        # e.g. 5_000
    n_train = n_total - n_val                # e.g. 45_000
    ...
    train_idx, val_idx = next(sss.split(np.zeros(n_total), labels))
    train_raw = Subset(raw_train, train_idx)
    val_raw   = Subset(raw_train, val_idx)

    # 3) Compute mean/std on train_raw (using your existing compute_mean_std)
    #    We need a ToTensor dataset just for stats:
    stats_ds = CIFAR10(root="data", train=True, download=False,
                       transform=transforms.ToTensor())
    stats_train = Subset(stats_ds, train_idx)
    mean, std = compute_mean_std(stats_train, batch_size)
    print(f"CIFAR-10 computed mean: {mean}, std: {std}")
    ...
    def get_augment():
        color_jitter = transforms.ColorJitter(0.8, 0.8, 0.8, 0.2)
        # small kernel since CIFAR is 32×32
        return transforms.Compose([
            transforms.RandomResizedCrop(CIFAR_IMAGE_SIZE, scale=(0.08, 1.0)),
    ...
    # 5) Wrap train in SimCLRDataset:
    simclr_train = SimCLRDataset(
        dataset=train_raw,
        transform=TwoCropsTransform(get_augment())
    )

    # 6) Wrap val & test in your LabeledEvalDataset:
    val_eval_ds  = LabeledEvalDataset(val_raw,  eval_tf)
    test_eval_ds = LabeledEvalDataset(raw_test, eval_tf)
    ...
    return train_loader, val_loader, test_loader, val_raw, NUM_CLASSES
...
def train_simclr(
    ...
    val_loader,          # yields (img, label) via LabeledEvalDataset
    ...
    feature_dim,
    num_classes,
    augment_transform,   # same augment used in SimCLRDataset
    wandb_run=None,
    scheduler=None,
    seed=SEED
):
    ...
    raw_val_subset = val_loader.dataset.subset
    ...
    for epoch in range(1, simclr_epochs + 1):
        # — contrastive training on train_loader —
        model.train()
        total_loss = 0.0
        ...
        with torch.no_grad():
            for v1, v2 in contrastive_val_loader:
                ...
                _, zv1 = model(v1)
                _, zv2 = model(v2)
                val_loss += criterion(zv1, zv2).item() * v1.size(0)
...
for seed in seeds:
    ...
    train_loader, val_loader, test_loader, val_subset_no_transform, num_classes = get_cifar_data_loaders(
    batch_size=CONFIG["BATCH_SIZE"],
    val_frac=0.1,
    seed=CONFIG["SEED"])
