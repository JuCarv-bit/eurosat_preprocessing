...
def run_linear_probe(model,
                     ...
                     epochs):
    """
    Train a fresh linear head on top of a *frozen* encoder, then eval it.
    This never mutates the encoder's mode or grad flags.
    """
    # 1) Create the head
    ...
    for _ in range(epochs):
        for imgs, labels in probe_train_loader:
            ...
            # Extract features without touching encoder weights or BN stats:
            with torch.no_grad():
                feats = model.encoder(imgs)
            # Forward + backward only through `head`:
            logits = head(feats)
    ...
    with torch.no_grad():
        for imgs, labels in probe_val_loader:
            imgs, labels = imgs.to(device), labels.to(device)
            feats  = model.encoder(imgs)      # still no grad
